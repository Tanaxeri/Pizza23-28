            ...
            switch (val)
            {
                case 23:
                    try
                    {
                        ...
                        command.CommandText = "SELECT fnev, COUNT(rendeles.fazon) FROM futar JOIN rendeles USING(fazon) GROUP BY fnev;";/*23.feldadat*/
                ...
                case 24:
                    try
                    {
                        ...
                        MySqlCommand command = connection.CreateCommand();
                        command.CommandText = "SELECT pnev, SUM(tetel.db) FROM pizza JOIN tetel USING(pazon) GROUP BY pnev ORDER BY SUM(tetel.db) DESC;";/*24.feldat*/
                        using (MySqlDataReader dr = command.ExecuteReader())
                        {
                ...
                case 25:
                    try
                    {
                        ...
                        MySqlCommand command = connection.CreateCommand();
                        command.CommandText = "SELECT vnev, SUM(pizza.par*tetel.db) FROM tetel JOIN rendeles USING(razon) JOIN vevo USING(vazon) JOIN pizza USING(pazon) GROUP BY vevo.vnev ORDER BY SUM(pizza.par*tetel.db) DESC;";/*25.feldat*/
                        using (MySqlDataReader dr = command.ExecuteReader())
                        {
                ...
                case 26:
                    try
                    {
                        ...
                        MySqlCommand command = connection.CreateCommand();
                        command.CommandText = "SELECT pizza.pnev, pizza.par FROM pizza WHERE pizza.par=(SELECT MAX(pizza.par) FROM pizza);";/*26.feldat*/
                        using (MySqlDataReader dr = command.ExecuteReader())
                        {
                ...
                case 27:
                    try
                    {
                        ...
                        MySqlCommand command = connection.CreateCommand();
                        command.CommandText = "SELECT futar.fnev, SUM(tetel.db) FROM rendeles JOIN tetel USING(razon) JOIN futar USING(fazon) GROUP BY futar.fnev ORDER BY SUM(tetel.db) DESC LIMIT 1;";/*27.feldat*/
                        using (MySqlDataReader dr = command.ExecuteReader())
                        {
